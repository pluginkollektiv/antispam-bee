...
class Antispam_Bee {
		...
		if ( defined( 'DOING_CRON' ) ) {
			...
			if ( self::_current_page( 'dashboard' ) ) {
				...
				if ( ! empty( $_GET['comment_status'] )
				     && 'spam' === $_GET['comment_status']
				     && ! self::get_option( 'no_notice' )
				) {
					/* Include file */
	...
	private static function _current_page( $page ) {
		switch ( $page ) {
			...
			case 'edit-comments':
				return ( ! empty( $GLOBALS['pagenow'] )
				         && 'edit-comments.php' === $GLOBALS['pagenow'] );
	...
	public static function init_row_meta( $input, $file ) {
		...
		return array_merge(
			$input,
			array(
				'<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=8CH5FPR88QYML" target="_blank" rel="noopener noreferrer">'
				. esc_html__( 'Donate', 'antispam-bee' ) . '</a>',
				'<a href="https://wordpress.org/support/plugin/antispam-bee" target="_blank" rel="noopener noreferrer">'
				. esc_html__( 'Support', 'antispam-bee' ) . '</a>',
			)
		);
	}
	...
	public static function add_dashboard_count( $items = array() ) {
		/* Skip */
		if ( ! current_user_can( 'manage_options' )
		     || ! self::get_option( 'dashboard_count' )
		) {
			return $items;
		}
	...
	public static function add_dashboard_chart() {
		...
		if ( ! current_user_can( 'publish_posts' )
		     || ! self::get_option( 'dashboard_chart' )
		) {
			return;
		}
	...
	public static function handle_incoming_request( $comment ) {
		...
		if ( strpos( $request_path, 'wp-comments-post.php' ) !== false
			...
			/* Is a trackback */
		           ...
		           && in_array(
			           self::get_key( $comment, 'comment_type' ),
			           $ping['types'],
			           true
		           )
		) {
	...
	public static function prepare_comment_field() {
		...
		/* Nur Beiträge */
		if ( ! is_singular()
		     && ! self::get_option( 'always_allowed' )
		) {
			return;
		}
	...
	private static function _verify_trackback_request( $comment ) {
		...
		/* BBCode Spam */
		if ( $options['bbcode_check']
		     && self::_is_bbcode_spam( $body )
		) {
			return array(
				'reason' => 'bbcode',
			);
		...
		/* IP != Server */
		if ( $options['advanced_check']
		     && self::_is_fake_ip( $ip, parse_url( $url, PHP_URL_HOST ) )
		) {
			return array(
				'reason' => 'server',
			);
		...
		/* DNSBL Spam */
		if ( $options['dnsbl_check']
		     && self::_is_dnsbl_spam( $ip )
		) {
			return array(
				'reason' => 'dnsbl',
			);
	...
	private static function _verify_comment_request( $comment ) {
		...
		/* Bereits kommentiert? */
		if ( $options['already_commented']
		     && ! empty( $email )
		     && self::_is_approved_email( $email )
		) {
			return;
		}
	...
	private static function _is_shortest_time() {
		...
		/**
		 * Threshold against which the comment action time is compared.
		 *
		 * A comment that is posted very quickly after loading a page is most
		 * probably a bot that just goes straight to the comment form for
		 * spamming. Set the threshold you want to check against to eliminate
		 * these bots.
		 *
		 * @param int $ab_action_time_limit Threshold in seconds below which
		 *                                  bots are assumed.
		 *                                  Defaults to 5 seconds.
		 */
		$time_threshold = apply_filters( 'ab_action_time_limit', 5 );

		/* Compare time values */
		if ( time() - $init_time < $time_threshold ) {
			return true;
		}
		...
		foreach ( $patterns as $pattern ) {
			...
			foreach ( $pattern as $field => $regexp ) {
				...
				$comment[ $field ] = ( function_exists( 'iconv' )
					? iconv( 'utf-8', 'utf-8//TRANSLIT', $comment[ $field ] )
					: $comment[ $field ] );
	...
	private static function _is_country_spam( $ip ) {
		...
		/* Get country code */
		$country = (string) wp_remote_retrieve_header( $response,
			'x-country-code' );
	...
	private static function _is_fake_ip( $client_ip, $client_host = false ) {
		...
		return false === strpos( $client_ip, self::_cut_ip( $ip_by_host ) );
	}
	 ...
	 */
	private static function _handle_spam_request(
		$comment,
		$reason,
		$is_ping = false
	) {
		...
		$ignore_reason = in_array(
			$reason,
			(array) $options['ignore_reasons'],
			true
		);
		...
		/* Typen behandeln */
		if ( $ignore_filter
		     && ( ( $ignore_type == 1 && $is_ping )
		          || ( $ignore_type == 2 && ! $is_ping ) )
		) {
			self::_go_in_peace();
		}
	 ...
	 * @change 2.5.7
	 *
	 * @hook   string antispam_bee_notification_subject Custom subject for
	 *         notification mails
	 *
	 * @param int $id ID des Kommentars
	 *
	 * @return int $id  ID des Kommentars
	 */
